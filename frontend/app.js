    ...
    pharmacies.forEach(p => {
      if (p.location && p.location.lat) {
        ...
        const infoWindow = new google.maps.InfoWindow({
          content: `<div style="font-family:Poppins,sans-serif;padding:8px;">
            <h3 style="color:#1a73e8;margin:0 0 6px">🏪 ${p.name}</h3>
            <p style="margin:0 0 4px">📍 ${p.address}</p>
            <p style="margin:0 0 4px">📞 ${p.phone || 'N/A'}</p>
            <p style="margin:0;color:#2e7d32;font-weight:600">✅ ${p.inventory.filter(i => i.inStock).length} medicines in stock</p>
          </div>`
        });
