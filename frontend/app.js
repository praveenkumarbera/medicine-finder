...
function findNearbyPharmacies() {
  ...
  btn.disabled = true;
  if (!navigator.geolocation) {
    document.getElementById('pharmacyList').innerHTML = '<p style="color:red">Geolocation not supported.</p>';
    ...
    pharmacies.slice(0,10).forEach((p,i) => {
      ...
      marker.addListener('click', () => {
        infoWindow.setContent(`<div style="font-family:Inter,sans-serif;padding:4px;max-width:200px">
          ...
          <div style="font-size:12px;color:#1a73e8;margin-top:4px">📏 ${p.distance<1?Math.round(p.distance*1000)+'m':p.distance.toFixed(1)+'km'} away</div>
        </div>`);
